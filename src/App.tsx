...
function App() {
    ...
    const [dataToUseOnPDfWithTypeA, setDataToUseOnPDfWithTypeA] =
        useState<{ [page: string]: string }>();
    const [dataToUseOnPDfWithTypeB, setDataToUseOnPDfWithTypeB] =
        useState<{ [page: string]: { x: number; y: number; str: string }[] }>();
    ...
    const [showLabelTypeError, setShowLabelTypeError] =
                ...
                .promise.then((pdf) => {
                    ...
                    Promise.all(allPromises).then((allPages) => {
                        ...
                        for (let j = 0; j < allPages.length; j++) {
                            allPagesTextContents.push(
                                allPages[j].getTextContent()
                            );
                        }

                        Promise.all(allPagesTextContents).then(
                            (allTextContents) => {
                                if (labelType === 'A') {
                                    const result: { [page: string]: string } =
                                        {};
                                    allTextContents.forEach(
                                        (textContent, index) => {
    ...
    async function createModyfiedPdf(): Promise<void> {
        if (
            !(pdfToEdit && (dataToUseOnPDfWithTypeA || dataToUseOnPDfWithTypeB))
        ) {
            return;
        }

        const ubuntuFontBytes = await fetch('ubuntu-font.ttf').then((res) =>
            res.arrayBuffer()
        );
    ...
    function shouldDisableGeneratePdfButton() {
        return !(
            checkedAllPages &&
            pdfToEdit !== undefined &&
            (dataToUseOnPDfWithTypeA !== undefined ||
                dataToUseOnPDfWithTypeB !== undefined) &&
            !showPdfFileError &&
            !showCsvFileError &&
            labelType
        );
        ...
        <div className="wrapper">
                ...
                <div className="container">
                    ...
                    <div className="mb-5 row">
                        <div className="form-check col">
                            ...
                            >
                                <img
                                    src="images/labels-type-A.png"
                                    className="label-type-image"
                                />
                            </label>
                        </div>
                        <div className="form-check col">
            ...
            {labelType && (
                <div className="container">
                    ...
                    <div className="mb-5 center">
                        <img
                            src={`images/labels-type-${labelType}.png`}
                            className="label-type-image"
                        />
                    </div>
                </div>
            ...
            {labelType && (
                <>
                    <div className="mb-5">
                        <label
                            htmlFor="translations-csv"
                            className="form-label"
                        >
                            Plik CSV z tłumaczeniami:
                            {translations && !showCsvFileError && (
                                <img
                    ...
                    <div className="mb-5">
                        ...
                        <input
                            ...
                            id="labels-pdf"
                            className="form-control"
                            onChange={pdfInputChange}
                            disabled={!translations || !labelType}
                        />
                        {showPdfFileError && (
                            <div className="error-hint">
                                Wczytaj poprawny plik PDF z etykietami!
                            </div>
                        )}
                        {totalPdfPages && !showPdfFileError && (
                            <div>
                                Wczytano plik zawierający {totalPdfPages} stron.
                            </div>
                        )}
                    </div>
                ...
                <div className="modal-dialog modal-xl modal-dialog-scrollable">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            <p>
                                Wybierz z menu opcję "Zapisz jako", a następnie
                                wybierz format pliku jako CSV rozdzielony
                                przecinkami:
                            </p>
                            <div className="wrapper-center">
                                <img
                                    src="images/how-save-csv.jpg"
                                    className="info-image"
                                />
                            </div>
                            <p>
                                Czasami może pojawić się dodatkowe okno z
                                informacją o używanych w pliku makrach,
                                formułach itp. Należy potwierdzić, że chcemy
                                zapisać plik:
                            </p>
                            <div className="wrapper-center">
                                <img
                                    src="images/confirm-save-csv.jpg"
                                    className="info-image"
                                />
                            </div>
                        </div>
